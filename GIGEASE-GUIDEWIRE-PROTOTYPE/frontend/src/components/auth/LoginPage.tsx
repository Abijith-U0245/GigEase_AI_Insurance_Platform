...
const LoginPage: React.FC = () => {
  ...
  const handleGetOTP = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('http://localhost:8000/send_otp_telegram', {
        method: 'POST',
      ...
      if (data.status === 'success') {
        ...
        setErrorMsg(data.message || "Unknown Telegram error");
      }
    } catch (e) {
      setLoading(false);
      ...
      <div className="w-full max-w-md bg-black md:border md:border-neutral-800 md:shadow-2xl md:rounded-3xl md:p-10 px-2">
        ...
        {role && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full mt-4">
            ...
            >
              {loading ? 'Transmitting...' : role === 'admin' ? 'Enter Admin Dashboard' : 'Get Telegram OTP'}
            </button>
          </motion.div>
        )}
        ...
        <p className="text-xs text-neutral-600 mt-6 text-center font-medium">
          By continuing you confirm Telegram consent
        </p>
