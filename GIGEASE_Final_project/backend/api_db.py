...
from ml_orchestrator import predict_worker_week
...
async def predict_latest(worker_id: str):
    ...
    if pred["claim_triggered"] == 1:
        rag_text = pred.get("rag_explanation") or ""
        explanation = {
            ...
            "reason_detail": (
                f"Income Rs.{pred_input.get('w_actual', 0):.0f} vs threshold "
                f"Rs.{float(pred.get('W_expected') or pred_input.get('w_expected') or 0) * 0.6:.0f}. "
                f"Claim Rs.{pred['claim_amount_inr']:.2f}. "
                f"Zone risk: {pred.get('zone_risk_score', 0):.2f}. "
                f"Fraud: {pred.get('fraud_action')} (score {pred.get('final_fraud_score', 0):.2f})."
            ),
            "next_premium": pred["next_week_premium_inr"],
            "loading_note": None,
            "fraud_note": ", ".join(pred.get("l1_flags") or []) or None,
...
async def simulate_claim_history(worker_id: str):
    ...
    for row in rows:
        ...
        if sample_rag_claim is None and int(pred.get("claim_triggered") or 0) == 1:
            ...
            sample_rag_claim = {
                "decision": "CLAIM APPROVED",
                "payout_inr": pred.get("claim_amount_inr", 0),
                "rag_narrative": rag_text,
                "fraud_action": pred.get("fraud_action"),
                "zone_risk_score": pred.get("zone_risk_score"),
            }
