...
const cardBase =
  'rounded-2xl border border-cyan-500/20 bg-gradient-to-br from-slate-900/80 via-slate-800/60 to-black/90 p-4 md:p-5 shadow-lg backdrop-blur-xl relative overflow-hidden transform hover:-translate-y-1 transition-all duration-300 hover:shadow-cyan-500/10 hover:border-cyan-400/40';
        ...
        <>
          <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-gray-400">GigEase control center — DEVTrails 2026</p>
            </div>
            <motion.button
              type="button"
              whileTap={{ scale: 0.98 }}
          ...
          <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {ICR_CARDS.map((c, idx) => {
              // Extract the base color name assuming it's like 'text-orange-400'
              const colorBase = c.color.split('-')[1] || 'cyan';
              return (
              <motion.div
                key={c.label}
                ...
                className={`${cardBase}`}
              >
                <div className={`absolute top-0 right-0 w-24 h-24 bg-${colorBase}-500/10 blur-[30px] rounded-full pointer-events-none`} />
                <div className="relative z-10">
                  <p className={`mb-1 text-xs font-semibold uppercase tracking-wider text-${colorBase}-400`}>{c.label}</p>
          ...
          <div className={`${cardBase} mb-6`}>
            <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-emerald-400">System Health</p>
            <div className="flex flex-wrap gap-x-6 gap-y-3">
              {['Kafka', 'Redis', 'PostgreSQL'].map(s => (
                <div key={s} className="flex items-center gap-3 bg-black/40 px-5 py-3 rounded-2xl border border-emerald-500/30">
                  <motion.div
                  ...
                  <span className="text-sm font-semibold text-white">
                    {s} <span className="text-emerald-500/50 font-normal">·</span> <span className="text-emerald-400">LIVE</span>
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className={`${cardBase}`}>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-fuchsia-400">Last Trigger Event</p>
            <p className="text-sm font-medium text-white bg-black/20 inline-block px-4 py-2 rounded-xl border border-white/5">STFI Flood <span className="text-fuchsia-500 px-2">—</span> Velachery <span className="text-fuchsia-500 px-2">—</span> <span className="text-fuchsia-200">04-Nov-2024 06:00 AM</span></p>
          </div>
        </>
      ) : (
